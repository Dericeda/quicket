...
const BookingForm = ({ event, onBookingSuccess }) => {
      ...
      if (savedData) {
        try {
          ...
          if (
            ...
            Array.isArray(parsedData.selectedSeats)
          ) {
            setSelectedSeats(parsedData.selectedSeats);
            setSeats(parsedData.selectedSeats.length);
          }
        } catch (error) {
  ...
  const handleBooking = async () => {
    ...
    if (selectedSeats.length === 0) {
      setStatus({
        type: "error",
        message: t("bookingForm.errors.noSeatsSelected"),
      });
      return;
    }
    ...
    if (selectedSeats.length > event.available_seats) {
      setStatus({
        type: "error",
        message: t("bookingForm.errors.tooManySeats", {
          count: event.available_seats,
        }),
      });
      return;
    }
  ...
  const handleSeatSelect = (seatId) => {
    setSelectedSeats((prevSeats) => {
      ...
      if (prevSeats.includes(seatId)) {
        ...
        setSeats(newSelectedSeats.length);
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setStatus({
          type: "success",
          message: t("bookingForm.success.message"),
        });
      ...
      <div className="booking-form">
        <div className="booking-form-header">
          ...
          <p className="booking-form-subtitle">
            {t("bookingForm.success.subtitle")}
          </p>
        </div>
        <div className="booking-form-content">
          <div className="success-animation">
            ...
            {/* Показываем забронированные места */}
            {selectedSeats.length > 0 && (
              <div className="success-seats">
                <h4>{t("bookingForm.selectedSeats")}:</h4>
                <div className="success-seat-tags">
                  {selectedSeats.map((seatId) => {
                    ...
                    if (isAstanaArena) {
                      ...
                      if (parts.length === 3) {
                        ...
                        displayText = `${t("stadium.sector")} ${subsector}, ${t(
                          "stadium.row"
                        )} ${row}, ${t("stadium.seat")} ${seat}`;
                      } else {
                        displayText = seatId;
                      }
                    } else {
                      const [row, number] = seatId.split("-");
                      displayText = t("bookingForm.rowSeat", {
                        row,
                        seat: number,
                      });
                    }
                    ...
                    return (
                      <span key={seatId} className="success-seat-tag">
                        {displayText}
                      </span>
                    );
                  })}
        ...
        {event.available_seats > 0 ? (
          <>
            <div className="booking-seats-toggle">
              ...
              >
                {showSeatSelection
                  ? t("seatSelection.hideSeatMap")
                  : t("seatSelection.showSeatMap")}
              </button>
            </div>
              ...
              <>
                  ...
                  <div className="seats-input">
                    <div className="seats-display">
                      ...
                      <span className="seats-text">
                        {t("seatSelection.seatsSelected")}
                      </span>
                    </div>
                    <span className="seats-available-text">
                      {t("bookingForm.availableSeats", {
                        count: event.available_seats,
                      })}
                    </span>
                  </div>
                ...
                {selectedSeats.length > 0 && (
                  <div className="selected-seats-display">
                    <h4>{t("seatSelection.yourSelectedSeats")}:</h4>
                        ...
                        return (
                          <span key={seatId} className="seat-tag">
                            ...
                            <button
                              ...
                              title={t("bookingForm.removeSeat")}
                              aria-label={t("bookingForm.removeSeat")}
                            >
                              ×
                            </button>
                          </span>
            ...
            <div className="price-calculation">
              ...
              <div className="price-row total">
                ...
                <span>
                  {selectedSeats.length * event.price} {t("currency.symbol")}
                </span>
              </div>
            </div>
