...
const StadiumSeatSelection = ({ event, selectedSeats, onSeatSelect }) => {
    ...
    try {
      ...
      sectorLetters.forEach((sectorLetter) => {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          const rowCount =
            sectorLetter === "A" || sectorLetter === "C" ? 15 : 12;
          for (let row = 1; row <= rowCount; row++) {
            const seatRow = [];
            // Создаем места в ряду
            const seatCount =
              sectorLetter === "B" || sectorLetter === "D" ? 15 : 20;
      ...
      setError("Ошибка при создании схемы стадиона");
      setLoading(false);
    }
  };
  ...
  const formatSeatId = (seatId) => {
    ...
    if (parts.length === 3) {
      ...
      return `Сектор ${subsector}, Ряд ${row}, Место ${seat}`;
    }
    return seatId;
  };
  ...
  return (
    <div className="stadium-selection-container">
      <h3 className="stadium-title">Выбор мест на стадионе "Astana Arena"</h3>
      ...
      <div className="stadium-overview">
        ...
        <div className="stadium-sectors">
          {sectors.map((sector) => (
            <div
              key={sector.id}
              className={`stadium-sector sector-${sector.id} ${
                activeSector === sector.id ? "active" : ""
              }`}
        ...
        <div className="active-sector-detail">
          ...
          <div className="subsector-tabs">
            {getActiveSectorObj().subsectors.map((subsector) => (
              <button
                ...
                onClick={() => {
                  document
                    .getElementById(`subsector-${subsector.id}`)
          ...
          <div className="subsectors-container">
            {getActiveSectorObj().subsectors.map((subsector) => (
              ...
              >
                <h5>Подсектор {subsector.name}</h5>

                <div className="subsector-rows">
                  {subsector.rows.map((row, rowIndex) => (
                    <div
                      key={`row-${subsector.id}-${rowIndex}`}
      ...
      <div className="selected-seats-summary">
        ...
        {selectedSeats.length > 0 ? (
          ...
          <p className="no-seats-selected">Нет выбранных мест</p>
        )}
      </div>
    </div>
  );
